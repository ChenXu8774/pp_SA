// $Id: saModuleSngmuonMing.C,v 1.6 2016/02/28 05:32:50 jinhuang Exp $                                                                                             
//
...
saModuleSngmuonMing::saModuleSngmuonMing(const std::string &name) :
  saModuleBase(name),                                         // muons
   ...
   _h_ST1Z_N(NULL),         // St1K multiple scattering angle
   _h_ST1Z_S(NULL),         // St1 hit Z

   _h_dA(NULL),         // St1 multiple scattering angle
   _h2_dA(NULL),        // St1 multiple scattering angle vs pZ

   _h_phi_1N(NULL),         // St1 phi angle
   _h_phi_1S(NULL),         // St1 phi angle

   _h_phi_2N(NULL),         // St2 phi angle
   _h_phi_2S(NULL),         // St2 phi angle

   _h_phi_3N(NULL),         // St3 phi angle
   _h_phi_3S(NULL),         // St3 phi angle

   _h_phi_13N(NULL),         // St1-3 phi angle
   _h_phi_13S(NULL),         // St1-3 phi angle

   _h2_phi_13N(NULL),         // St1-3 phi angle
   _h2_phi_13S(NULL),         // St1-3 phi angle

   _h_phi_23N(NULL),         // St1-3 phi angle
   _h_phi_23S(NULL),         // St1-3 phi angle

   _h2_phi_23N(NULL),         // St1-3 phi angle
   _h2_phi_23S(NULL),         // St1-3 phi angle

  // -- RPC stuff
   _h_Rpc1DCA_N(NULL), // RPC1 DCA north
   _h_Rpc1DCA_S(NULL), // RPC1 DCA south

   _h_Rpc3DCA_N(NULL), // RPC3 DCA north
   _h_Rpc3DCA_S(NULL), // RPC3 DCA south


  //for comparision with run11/12 style W->muon analysis
   _h_dw23N_p(NULL),         // charge=+(NULL), St2-3 rescaled phi angle == pT*sin(theta)*dPhi23
   _h_dw23N_m(NULL),         // charge=-(NULL),St2-3 rescaled phi angle == pT*sin(theta)*dPhi23
   _h_dw23S_p(NULL),         // St2-3 rescaled phi angle
   _h_dw23S_m(NULL),         // St2-3 rescaled phi angle

   _h_dw23N_PT10_p(NULL),         // charge=+(NULL), St2-3 rescaled phi angle == pT*sin(theta)*dPhi23
   _h_dw23N_PT10_m(NULL),         // charge=-(NULL),St2-3 rescaled phi angle == pT*sin(theta)*dPhi23
   _h_dw23S_PT10_p(NULL),         // St2-3 rescaled phi angle
   _h_dw23S_PT10_m(NULL),         // St2-3 rescaled phi angle

   _h_dw23N_PT15_p(NULL),         // charge=+(NULL), St2-3 rescaled phi angle == pT*sin(theta)*dPhi23
   _h_dw23N_PT15_m(NULL),         // charge=-(NULL),St2-3 rescaled phi angle == pT*sin(theta)*dPhi23
   _h_dw23S_PT15_p(NULL),         // St2-3 rescaled phi angle
   _h_dw23S_PT15_m(NULL),         // St2-3 rescaled phi angle

   _h_dw23N_PT20_p(NULL),         // charge=+(NULL), St2-3 rescaled phi angle == pT*sin(theta)*dPhi23
   _h_dw23N_PT20_m(NULL),         // charge=-(NULL),St2-3 rescaled phi angle == pT*sin(theta)*dPhi23
   _h_dw23S_PT20_p(NULL),         // St2-3 rescaled phi angle
   _h_dw23S_PT20_m(NULL),         // St2-3 rescaled phi angle

  // -- eta for W/Z analysis, for distribution check only, with fine bins --
   _hx_etaW10(NULL),
   _hx_etaW10_p(NULL),      // Q+ high pT>10 tracks
   _hx_etaW10_m(NULL),      // Q- high pT>10 tracks

   _hx_etaW15(NULL),
   _hx_etaW15_p(NULL),      // Q+ high pT>15 tracks
   _hx_etaW15_m(NULL),      // Q- high pT>15 tracks

   _hx_etaW20(NULL),
   _hx_etaW20_p(NULL),      // Q+ high pT>20 tracks
   _hx_etaW20_m(NULL),      // Q- high pT>20 tracks
