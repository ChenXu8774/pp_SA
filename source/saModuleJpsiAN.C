...
saModuleJpsiAN::saModuleJpsiAN(const std::string &name, bool doControlData) :
	saModuleBase(name),                                     //
	_sah_xF_os_sig_n(NULL), _sah_xF_os_sig_s(NULL),
	_sah_xF_os_lsb_n(NULL), _sah_xF_os_lsb_s(NULL),
        ...
        _sah_pT_os_lsb_n(NULL), _sah_pT_os_lsb_s(NULL)
{
	...
	JPSI_LSB_MASS_MIN = 1.5;
	JPSI_LSB_MASS_MAX = 2.4;
...
//	_pT_bins[3] = 10.;
	...
	_pT_MinJpsiMs[NORTH][0] =2.882;
	_pT_MaxJpsiMs[NORTH][0] =3.417;
	_pT_MinJpsiMs[NORTH][1] =2.872;
        _pT_MaxJpsiMs[NORTH][1] =3.432;

        _pT_MinJpsiMs[SOUTH][0] =2.856;
        _pT_MaxJpsiMs[SOUTH][0] =3.379;
        _pT_MinJpsiMs[SOUTH][1] =2.791;
        _pT_MaxJpsiMs[SOUTH][1] =3.453;
	
	_xF_MinJpsiMs[NORTH] = 2.879;
        _xF_MaxJpsiMs[NORTH] = 3.443;

        _xF_MinJpsiMs[SOUTH] = 2.842;
        _xF_MaxJpsiMs[SOUTH] = 3.403;
...
int saModuleJpsiAN::init(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
	...
	_sah_xF_os_lsb_n = makesaHist_pT_phi(hm,makeTHist_xF_phi("_sah_xF_os_lsb_n","same sign, lower side band, north"));
        _sah_xF_os_lsb_s = makesaHist_pT_phi(hm,makeTHist_xF_phi("_sah_xF_os_lsb_s","same sign, lower side band, south"));
        _sah_xF_os_sig_n = makesaHist_pT_phi(hm,makeTHist_xF_phi("_sah_xF_os_sig_n","opposite sign, signal region, north"));
        _sah_xF_os_sig_s = makesaHist_pT_phi(hm,makeTHist_xF_phi("_sah_xF_os_sig_s","opposite sign, signal region, south"));
...
int saModuleJpsiAN::event(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
	...
	for (unsigned int imuon = 0; imuon < dimuons->get_nDiMuons(); imuon++)
	{
		...
		if(!((triglv1->get_lvl1_trigscaled() & 0x00400000)||(triglv1->get_lvl1_trigscaled() & 0x00100000)||(triglv1->get_lvl1_trigscaled() & 0x00800000)||(triglv1->get_lvl1_trigscaled() & 0x00200000))) continue;

//		if(!(dimuon->get_Tr0_pz()>0&&((triglv1->get_lvl1_trigscaled() & 0x00400000)||(triglv1->get_lvl1_trigscaled() & 0x00100000)))) continue;
//		if(!(dimuon->get_Tr0_pz()<0&&((triglv1->get_lvl1_trigscaled() & 0x00800000)||(triglv1->get_lvl1_trigscaled() & 0x00200000)))) continue; 

		enum EVENT_TYPE type = BAD_EVENT;

		float mass = dimuon->get_mass();
		float rapidity = dimuon->get_rapidity();
                float x1 = mass/200.*TMath::Exp(rapidity);
                float x2 = mass/200*TMath::Exp(-rapidity);
                float xF = x1 - x2;

		enum ARM_TYPE arm;
		arm = (dimuon->get_rapidity() > 0) ? NORTH : SOUTH;
		
		cout<<"TEST....................."<<dimuon->get_Tr0_pz()<<endl;
		...
		if (xF >0 && mass<JPSI_LSB_MASS_MAX && mass>JPSI_LSB_MASS_MIN)
                _sah_xF_os_lsb_n->Fill(ep, abs(xF), phi);

                if (xF <0 && mass<JPSI_LSB_MASS_MAX && mass>JPSI_LSB_MASS_MIN)
                _sah_xF_os_lsb_s->Fill(ep, abs(xF), phi);

                if (xF >0 && mass<_xF_MaxJpsiMs[0] && mass>_xF_MinJpsiMs[0])
                _sah_xF_os_sig_n->Fill(ep, abs(xF), phi);

                if (xF <0 && mass<_xF_MaxJpsiMs[1] && mass>_xF_MinJpsiMs[1])
                _sah_xF_os_sig_s->Fill(ep, abs(xF), phi);
	...
	//		)
		{
			fout
				...
				<<setw(10)<<" beamclk0: "<<setw(15)<<triglv1->get_lvl1_beam_clk(0)
				...
				<<setw(10)<<" mass: "<<setw(10)<<dimuon->get_mass()
				<<setw(6)<<" pT: "<<setw(8)<<dimuon->get_pT()
...
//	if(!(sqrt(dimuon->get_X0()*dimuon->get_X0()+dimuon->get_Y0()*dimuon->get_Y0())<2.)) return false;
	if(!(dimuon->get_charge()==0)) return false;

	if(dimuon->get_Tr0_pz()>0)
	{if(!(dimuon->get_Tr0_DG0()<25. && dimuon->get_Tr1_DG0()<25.)) return false;}

	if(dimuon->get_Tr0_pz()<0)
        {if(!(dimuon->get_Tr0_DG0()<30. && dimuon->get_Tr1_DG0()<30.)) return false;}
	...
	if(!(dimuon->get_mass()>1.0 && dimuon->get_mass()<4.0)) return false;
...
{
	TH1 * h = new TH2F(name.data(),title.data(), 2, _pT_bins,16, -1.*TMath::Pi(), TMath::Pi());
	return h;
}

TH1 *saModuleJpsiAN::makeTHist_xF_phi(
                std::string name,
                std::string title
                )
{
        TH1 * h = new TH2F(name.data(),title.data(), 2, _xF_bins,16, -1.*TMath::Pi(), TMath::Pi());
        return h;
}
