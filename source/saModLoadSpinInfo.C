...
saModLoadSpinInfo::saModLoadSpinInfo(const std::string &name) :
    ...
    _h_lumi(NULL), _use_spin_QA(false), _costum_flag(saHist::DUMMY)
{
...
saModLoadSpinInfo::init(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
  ...
  _h_lumi = new saHist(h_lumi, saHist::DEFAULT_FLAG_LUMI | _costum_flag, Verbosity() - 1);
  ...
  for (int crossing = 0; crossing < saEventProperty::N_CROSSING; crossing++)
    {
      ...
      saEventProperty::sa_single_spin_state state_b = get_single_spin_state(crossing,"BLUE");
      saEventProperty::sa_single_spin_state state_y = get_single_spin_state(crossing,"YELLOW");
      ...
      ep_tmp.set_blue_spin_state(state_b);

      ep_tmp.set_yellow_spin_state(state_y);
...
//    event_num = _sync -> EventNumber();
  ...
  ep->set_blue_spin_state(get_single_spin_state(crossing,"BLUE"));

  ep->set_yellow_spin_state(get_single_spin_state(crossing,"YELLOW"));
...
saEventProperty::sa_single_spin_state
saModLoadSpinInfo::get_single_spin_state(int crossing, string name)
{
  ...
  if (_spin_cont->GetBadBunchFlagPHENIX(crossing))
    {
      ...
      return saEventProperty::SINGLE_UNKNOWN;
    }
  ...
  //! Get spin states
  saEventProperty::sa_single_spin_state state = saEventProperty::SINGLE_UNKNOWN;
  ...
  if(name == "BLUE")
    {
      if (sb == 1)
        state = saEventProperty::P;
      if (sb == -1)
        state = saEventProperty::M;
    }
  else if(name == "YELLOW")
    {
      if (sy == 1)
  ...
  if (Verbosity() >= 2)
    cout << "saModLoadSpinInfo::get_single_spin_state - INFO - spin_state["
        << crossing << "] = "<< state
        <<" | PHENIX Crossing = "<< crossing << endl;
  return state;
}
