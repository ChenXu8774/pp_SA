...
saHist::Init(TH1 * h)
{
  ...
  if (get_flag(BLUE_SPIN_DEPENDENT))
    {
      // for spin dependent fillings

      for (saEventProperty::sa_single_spin_state state = saEventProperty::P;
          state <= saEventProperty::SINGLE_UNKNOWN; //
          state = static_cast<saEventProperty::sa_single_spin_state>(state + 1))
        {
          const string sufix = saEventProperty::get_single_spin_state_name(state) + "U";
          insert_clone_hist(sufix);
        }
    }

  if (get_flag(YELLOW_SPIN_DEPENDENT))
    {
      // for spin dependent fillings
      ...
      for (saEventProperty::sa_single_spin_state state = saEventProperty::P;
          ...
          state = static_cast<saEventProperty::sa_single_spin_state>(state + 1))
        {
          const string sufix = "U" + saEventProperty::get_single_spin_state_name(state);
          insert_clone_hist(sufix);
        }
    }
...
saHist::Print(Option_t *option) const
{
  ...
  if (get_flag(BLUE_SPIN_DEPENDENT))
    cout << tab << "\t" << "Blue Spin Asymmetry analysis mode" << endl;
  else
    cout << tab << "\t" << "Non-Blue-Spin-Asymmetry analysis mode" << endl;

  if (get_flag(YELLOW_SPIN_DEPENDENT))
    cout << tab << "\t" << "Yellow Spin Asymmetry analysis mode" << endl;
  else
    cout << tab << "\t" << "Non-Yellow-Spin-Asymmetry analysis mode" << endl;
  ...
  if (get_flag(BLUE_SPIN_DEPENDENT))
    {
      ...
      saEventProperty::sa_single_spin_state state = ep->get_blue_spin_state();
      const string name = saEventProperty::get_single_spin_state_name(state) + "U";
  ...
  if (get_flag(YELLOW_SPIN_DEPENDENT))
    {
      ...
      saEventProperty::sa_single_spin_state state = ep->get_yellow_spin_state();
      const string name = "U" + saEventProperty::get_single_spin_state_name(state);
...
TH1 *
saHist::getHisto(const saEventProperty::sa_single_spin_state state) const
{
  if(get_flag(BLUE_SPIN_DEPENDENT))
    return getHisto(saEventProperty::get_single_spin_state_name(state)+"U");
  else if(get_flag(YELLOW_SPIN_DEPENDENT))
    return getHisto("U" + saEventProperty::get_single_spin_state_name(state));
  else
    {
      cout << "saHist::" << _name
          << "::getHisto - Error - try to access spin dependent histogram for non-spin-dependent object "
...
//! Calculate asymmetry according to luminorsity histogram h_lumi
bool
saHist::CalcSingleSpinAsymmetry(saHist * h_lumi, enu_lumi_id lumi_type)
{
  //RemoveAsymmetryHisto();
  ...
  if (get_flag(BLUE_SPIN_DEPENDENT))
    status &= CalcBlueSpinAsymmetry_Work(h_lumi, lumi_type);
  if (get_flag(YELLOW_SPIN_DEPENDENT))
    status &= CalcYellowSpinAsymmetry_Work(h_lumi, lumi_type);
      ...
      if (ref.IsValid() && sub_hist)
        {
          ...
          if (get_flag(BLUE_SPIN_DEPENDENT))
            status &= sub_hist->CalcBlueSpinAsymmetry_Work(h_lumi, lumi_type);
          if (get_flag(YELLOW_SPIN_DEPENDENT))
            status &= sub_hist->CalcYellowSpinAsymmetry_Work(h_lumi, lumi_type);
        }
    }
...
//! Calculate asymmetry according to luminorsity histogram h_lumi, DO NOT iterate through sub-histograms
bool
saHist::CalcBlueSpinAsymmetry_Work(saHist * h_lumi, enu_lumi_id lumi_type)
{
  ...
  !(this->getHisto("BASE")) or //
    ...
    {
      cout << "saHist::" << _name
          << "::CalcAsymmetry - Error - invalid single spin info." << endl;
      this->Print();
      return false;
    }
  ...
  if ( //
  !(h_lumi->getHisto("BASE")) or //
      !(h_lumi->getHisto("PU")) or //
      !(h_lumi->getHisto("MU")) or //
      !(h_lumi->getHisto("Pol_Blue"))
      )
    {
      cout << "saHist::" << _name
          << "::CalcAsymmetry - Error - invalid luminorsity info:" << endl;
  ...
  insert_clone_hist("A_L_Blue_SINGLE");
  insert_clone_hist("YIELD_SINGLE");

  const TH1 * h_NPU = getHisto("PU");
  const TH1 * h_NMU = getHisto("MU");


  const TH1 * h_RPU = h_lumi->getHisto("PU");
  const TH1 * h_RMU = h_lumi->getHisto("MU");
  const TH1 * h_RSUM = h_lumi->getHisto("SUM");

  const double RPU = h_RPU->GetBinContent(lumi_type);
  const double RMU = h_RMU->GetBinContent(lumi_type);
  const double RSUM = h_RSUM->GetBinContent(lumi_type);
  ...
  cout << "saHist::" << _name << "::CalcAsymmetry polarization (BLUE) = (" //
      << POL_BLUE << ")" << endl;

  TH1 * h_A_L_Blue = getHisto("A_L_Blue_SINGLE");
  TH1 * h_YIELD = getHisto("YIELD_SINGLE");
      ...
      for (int z = 1; z <= n_bin_z; z++)
        {
          /*
           ...
           Y = 0.5 * (NPU/RPU + NMU/RMU);
           AB = (NPU/RPU - NMU/RMU) / (NPU/RPU + NMU/RMU)
           */

          const double NPU = h_NPU->GetBinContent(x, y, z);
          const double NMU = h_NMU->GetBinContent(x, y, z);

          double A_L_Blue = (NPU/RPU - NMU/RMU) / (NPU/RPU + NMU/RMU);

          double YIELD = 0.5 * (NPU/RPU + NMU/RMU);

          double A_L_Blue_ERR2 = 4.
              * (NMU * NPU * RMU * RPU
                  * (NPU * RMU * RPU + NMU * (RMU * RPU + NPU * (RMU + RPU))))
              / pow(NPU * RMU + NMU * RPU, 4);


          double YIELD_ERR2 = 0.25 * (pow(NMU,2)/pow(RMU,3) + NMU/pow(RMU,2) + (NPU*(NPU + RPU))/pow(RPU,3));
...
//! Calculate asymmetry according to luminorsity histogram h_lumi, DO NOT iterate through sub-histograms
bool
saHist::CalcYellowSpinAsymmetry_Work(saHist * h_lumi, enu_lumi_id lumi_type)
{
  ...
  if ( //
  !(h_lumi->getHisto("BASE")) or //
      !(h_lumi->getHisto("UP")) or //
      !(h_lumi->getHisto("UM")) or //
      !(h_lumi->getHisto("Pol_Yellow"))
      )
    {
      cout << "saHist::" << _name
  ...
  insert_clone_hist("A_L_Yellow_SINGLE");
  insert_clone_hist("YIELD_SINGLE");

  const TH1 * h_NUP = getHisto("UP");
  const TH1 * h_NUM = getHisto("UM");


  const TH1 * h_RUP = h_lumi->getHisto("UP");
  const TH1 * h_RUM = h_lumi->getHisto("UM");
  const TH1 * h_RSUM = h_lumi->getHisto("SUM");

  const double RUP = h_RUP->GetBinContent(lumi_type);
  const double RUM = h_RUM->GetBinContent(lumi_type);
  const double RSUM = h_RSUM->GetBinContent(lumi_type);
  ...
  cout << "saHist::" << _name << "::CalcAsymmetry polarization (YELLOW) = (" //
      << POL_YELLOW << ")" << endl;

  TH1 * h_A_L_Yellow = getHisto("A_L_Yellow_SINGLE");
  TH1 * h_YIELD = getHisto("YIELD_SINGLE");
      ...
      for (int z = 1; z <= n_bin_z; z++)
        {
          /*
           ...
           NUP = RUP*Y*(1 + AY)
           NUM = RUM*Y*(1 - AY)
           AY = (NUP/RUP - NUM/RUM) / (NUP/RUP + NUM/RUM)
           Y = 0.5 * (NUP/RUP + NUM/RUM);
           */

          const double NUP = h_NUP->GetBinContent(x, y, z);
          const double NUM = h_NUM->GetBinContent(x, y, z);

          double A_L_Yellow = (NUP/RUP - NUM/RUM) / (NUP/RUP + NUM/RUM);

          double YIELD = 0.5 * (NUP/RUP + NUM/RUM);

          double A_L_Yellow_ERR2 = 4.
              * (NUM * NUP * RUM * RUP
                  * (NUP * RUM * RUP + NUM * (RUM * RUP + NUP * (RUM + RUP))))
              / pow(NUP * RUM + NUM * RUP, 4);


          double YIELD_ERR2 = 0.25 * (pow(NUM,2)/pow(RUM,3) + NUM/pow(RUM,2) + (NUP*(NUP + RUP))/pow(RUP,3));
