// $Id: saModuleDimuonDarshana.C created by jinhuang modified for Drell-Yan Analysis By Darshana Perera 
...
#include "saModuleDimuonDarshana.h"

using namespace std;

saModuleDimuonDarshana::saModuleDimuonDarshana(const std::string &name) :
	saModuleBase(name),
        /* _h_mass(NULL),
...
saModuleDimuonDarshana::~saModuleDimuonDarshana()
{

}

//! global initialization
int saModuleDimuonDarshana::init(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
//-------------------------------------------------------------------
/*	TH1F * h_pT_os_N = new TH1F("pT_os_sig_n","opposite sign, signal region, north", 3, _pT_bins);
	...
	TH1F * h_pT_os_S = new TH1F("pT_os_sig_s","opposite sign, signal region, south", 3, _pT_bins);
	_h_pT_os_S = new saHist(h_pT_os_S,saHist::DEFAULT_FLAG | saHist::RUN_PROPERTY | 
        saHist::FILL_PROPERTY| saHist::CROSSING_CHECKS,Verbosity());
	hm.get()->registerHisto(_h_pT_os_S);

	TH1F * h_pT_sm_N = new TH1F("pT_ss_n","same sign, signal region, north", 3, _pT_bins);
	_h_pT_sm_N = new saHist(h_pT_sm_N,saHist::DEFAULT_FLAG | saHist::RUN_PROPERTY | 
        saHist::FILL_PROPERTY| saHist::CROSSING_CHECKS,Verbosity());
	hm.get()->registerHisto(_h_pT_sm_N);

	TH1F * h_pT_sm_S = new TH1F("pT_ss_s","same sign, signal region, south", 3, _pT_bins);
...
*/
  ...
  if (flags)
      ...
      if (!node)
        {
          ...
          cout
              << "saModuleDimuonDarshana::Init failed to create saEventProperty Node"
              << endl;
          return ABORTRUN;
    ...
    {
      cout << "saModuleDimuonDarshana::Init failed to create saEventProperty"
          << endl;
      return ABORTRUN;
    }
...
//! Run initialization
int saModuleDimuonDarshana::init_run(PHCompositeNode *topNode,sa_hist_mangager_ptr hm)
{
  return EVENT_OK;
}

//! event method
int saModuleDimuonDarshana::event(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
//---------------------------------------------------------------------------------------------------------
   dimuons = findNode::getClass<DiMuonContainer>(topNode,"DiMuonContainer");
  if (!dimuons)
    {
      cout<< "saModuleDimuonDarshana:: DiMuonContainer - ERROR - not in Node Tree"<< endl;
      return ABORTRUN;
    }
//---------------------------------------------------------------------------------------------------------
  ...
  if (!ep)
    {
      cout<< "saModuleDimuonDarshana::event - ERROR  - Cannot find EventProperty node in Top Node"<< endl;
      return ABORTRUN;
    }
//---------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------
  if (Verbosity() >= 2)
    {
      cout << "saModuleDimuonDarshana::event - INFO  - ";
      ep->identify();
    }
...
//---------------------------------------------------------------------------------------------------------
  ...
  if (!flag)
    {
      cout << "saModuleDimuonDarshana::event - SimpleDimuonFlag not in Node Tree"<< endl;
      return ABORTRUN;
    }
...
//---------------------------------------------------------------------------------------------------------
	...
	if (!triglv1)
	{
		cout << "saModuleDimuonDarshana:: TrigLvl1 - ERROR - not in Node Tree"<< endl;
		return ABORTRUN;
	}
//---------------------------------------------------------------------------------------------------------
...
bool saModuleDimuonDarshana::passesCuts() const{
  // Cuts
...
int saModuleDimuonDarshana::GetNumberofTracklets(DSTReader *fvtx_trk_map, const DiMuon *dimuon)
{
	if(!fvtx_trk_map){
...
//! global termination
int saModuleDimuonDarshana::end(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
  ...
  //calculate asymmetry with relative lumi of BbcNoCutPHENIX
  /*_h_pT_os_N->CalcAsymmetry(hm.get()->getHisto_DefaultLumi(),saHist::BbcNoCutPHENIX);
  _h_pT_os_S->CalcAsymmetry(hm.get()->getHisto_DefaultLumi(),saHist::BbcNoCutPHENIX);

  _h_pT_sm_N->CalcAsymmetry(hm.get()->getHisto_DefaultLumi(),saHist::BbcNoCutPHENIX);
  _h_pT_sm_S->CalcAsymmetry(hm.get()->getHisto_DefaultLumi(),saHist::BbcNoCutPHENIX);
