...
public:
  ...
  enum en_flag
  {
    ...
    //! Blue Spin dependant flag
    BLUE_SPIN_DEPENDENT = 1 << 9,

    //! Yellow Spin dependant flag
    YELLOW_SPIN_DEPENDENT = 1 << 10,
...
public:
  ...
  //! get the histogram (TH1) corresponding to a single spin state
  TH1*
  getHisto(const saEventProperty::sa_single_spin_state state) const;
...
public:
  ...
  virtual
  bool
  CalcSingleSpinAsymmetry(saHist* h_lumi, enu_lumi_id lumi_type);
...
protected:
  ...
  virtual
  bool
  CalcBlueSpinAsymmetry_Work(saHist* h_lumi, enu_lumi_id lumi_type);

  //! Calculate asymmetry according to luminorsity histogram h_lumi, DO NOT iterate through sub-histograms
  //@return true if successful
  virtual
  bool
  CalcYellowSpinAsymmetry_Work(saHist* h_lumi, enu_lumi_id lumi_type);
