...
public:
  ...
  identify(std::ostream& os = std::cout) const
  {
    std::cout << "This is " << this->ClassName() << " with run = "
        << get_run_number() //
        << " spin state = " << (int) get_spin_state()
        << " blue spin state = " << (int) get_blue_spin_state()
        << " yellow spin state = " << (int) get_yellow_spin_state() << std::endl;
  }
...
public:
  ...
  enum sa_single_spin_state
  {
    P, M, SINGLE_UNKNOWN, SINGLE_SUM
  };
  ...
  static const std::string
  get_single_spin_state_name(sa_single_spin_state state)
  {
    switch (state)
      {
    case P:
    ...
    case M:
      return std::string("M");
    case SINGLE_SUM:
      return std::string("SINGLE_SUM");
    default:
      return std::string("SINGLE_UNKNOWN");
      }
    return std::string("SINGLE_UNKNOWN");
  }
  ...
  virtual sa_single_spin_state
  get_blue_spin_state() const
  {
  ...
  virtual
  void
  set_blue_spin_state(sa_single_spin_state spinState)
  {
  }

  virtual sa_single_spin_state
  get_yellow_spin_state() const
  {
    return SINGLE_UNKNOWN;
  }
  ...
  virtual
  void
  set_yellow_spin_state(sa_single_spin_state spinState)
  {
  }
protected:
